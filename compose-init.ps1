...
Param (
    ...
    $EnvTemplateFilePath = ".\.env.template",
    ...
    [string]
    $SitecoreGalleryRepositoryLocation = "https://sitecore.myget.org/F/sc-packages/api/v3/index.json",
...
function Invoke-ComposeInit {
    ...
    if(!(Test-Path $EnvTemplateFilePath)) {
        throw "Could not find '$EnvTemplateFilePath'"
    }
    Copy-Item $EnvTemplateFilePath $EnvFilePath -Force
    ...
    $envVariablesTable = @{ 
        ...
        "REPORTING_API_KEY" = Get-SitecoreRandomString 32 -DisallowSpecial
...
$logFilePath = Join-Path -path (Split-Path -Parent $MyInvocation.MyCommand.Path) -ChildPath "\docker\data\logs\compose-init-$(Get-date -f 'yyyyMMddHHmmss').log";
